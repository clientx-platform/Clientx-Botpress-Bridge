...
var (
	...
	BotId = ""
	// ClientxURL URL for Clientx installation
	ClientxURL = ""
	// ClientxBotToken Clientx Bot token from database
	ClientxBotToken = os.Getenv("clientxBotToken")
...
func handler(request events.APIGatewayProxyRequest) (events.APIGatewayProxyResponse, error) {
	...
	if req.MessageType == "incoming" || req.ContentAttributes.SubmittedValue != nil {
		...
		var clientxResponse *http.Response

		clientxChan := make(chan *http.Response)
		for _, response := range botMessages.Responses {
			//log.Printf("Bot responses: %v", response)
			if len(response.Text) > 0 {
				requestBody = SendToClientx(req.Conversation.Id, response, req.Conversation.Meta.Sender.CustomAttributes, response.Choices, response.Type)
			} else {
				requestBody = SendToClientx(req.Conversation.Id, response, req.Conversation.Meta.Sender.CustomAttributes, response.Choices, response.Type)

			}

			go SendPostAsync(requestBody, clientxChan)
			clientxResponse = <-clientxChan
			//log.Println(clientxResponse)
		}
		defer clientxResponse.Body.Close()
...
func SendToClientx(conversationId int, response Response, customAttributes CustomAttributes, choices []Choice, messageType string) *http.Request {
	var postBody []byte
	var err error
	...
	urlString := ClientxURL + "/api/v1/accounts/1/conversations/" + strconv.Itoa(conversationId) + "/messages"
	req, err := http.NewRequest(http.MethodPost, urlString, bytes.NewReader(postBody))
	req.Header.Add("Content-Type", "application/json")
	req.Header.Add("api_access_token", ClientxBotToken)
	if err != nil {
		log.Println("Error in sending to clientx...")
